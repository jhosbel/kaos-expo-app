...
import { CREATE_USER_DEPOSIT } from "@/graphql/mutations";
...
const RechargePage = () => {
  ...
  const [createUserDeposit] = useMutation(CREATE_USER_DEPOSIT);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await createUserDeposit({
        variables: {
          createUserDepositInput: {
            userId: Number(data?.userByEmail?.id),
            deposit: amount,
            depositImage: image,
          },
        },
      });
      console.log("Deposit created:", result.data.createUserDeposit);
      refetch();
      closeAll();
      setAmount("");
      setImage(null);
    } catch (error) {
      console.error("Error al hacer el deposito:", error);
    }
  };
  ...
  if (error) {
    console.error("Error en la consulta RechargePage: ", error);
    return <ErrorPage />;
  }
    ...
    <View style={{ height: "100%" }}>
      ...
      <View style={{ height: "100%", alignItems: "center", gap: 20 }}>
        ...
        {selectedValue === "binance" ? (
          ...
          >
            ...
            >
              A continuacion, el id al cual tiene que transferir el dinero para
              hacer la recarga
            </Text>
            <Text
            ...
            >
              Recuerde tomar capture de su transaccion para que podamos
              verificar su deposito
            </Text>
          </View>
          ...
          >
            ...
            >
              A continuacion los datos requieridos para la trasferencia
            </Text>
            <View
      ...
      >
        <View style={{ gap: 15, alignItems: "center" }}>
          ...
          {selectedValue === "binance" ? (
            <View>
              <Text style={{ fontSize: 15 }}>{`Metodo de pago: ${
                selectedValue === "binance" ? "Binance" : null
              }`}</Text>
              <Text
                style={{ fontSize: 15 }}
              >{`Monto a recargar: ${amount}`}</Text>
              <Text
                style={{
                  width: 320,
                  fontSize: 20,
            ...
            <View style={{ alignItems: "center" }}>
              <Text style={{ fontSize: 15 }}>{`Metodo de pago: ${
                selectedValue === "pagomovil" ? "Pago Movíl" : null
              }`}</Text>
              <Text
                style={{ fontSize: 15 }}
              >{`Monto a recargar: ${amount}`}</Text>
              <Text
                style={{ fontSize: 15 }}
              >{`Codigo del banco: ${codeBank}`}</Text>
              <Text
                style={{ fontSize: 15 }}
              >{`Numero de cedula: ${dniNum}`}</Text>
              <Text
                style={{ fontSize: 15 }}
              >{`Telefono asociado: ${phoneUser}`}</Text>
              <Text
                style={{
                  width: 320,
                  fontSize: 20,
      ...
      <SmallModalComponent isVisible={imgLoad} setIsVisible={setimgLoad}>
